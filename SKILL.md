...
引導/協助使用者開啟 [01-antigravity-lazypack.md](file:///Users/arrywu/Library/CloudStorage/GoogleDrive-icestone0128@gmail.com/我的雲端硬碟/antigravity_installation/01-antigravity-lazypack.md) 的「四、全域技能健檢與三方同步驗證」：
1. 驗證全域 Skills / MCP / CLI 依賴可用性（包含 Git/gh, Node/npm/playwright, Python/pip/poppler/yt-dlp, FFmpeg, mcpvault，以及驗證全域生圖技能 `image-generator` 是否正常運行）。
...
3. 遇到任何沙盒限制時，將限制新增到沙盒規則中（包含將 **PIP、NPM、Python 及其執行與工作路徑**完整列為沙盒例外 `writable_roots`）。
4. **若有任何問題，直接參考 `Github Codex Installation 的 lazy pack 路徑` 下的對應文件。所有技能的完整修復說明都在該安裝 Repo 下的整個 `lazy-pack` 資料夾（對應本地路徑：[codex_installation/lazy-pack](file:///Users/arrywu/Library/CloudStorage/GoogleDrive-icestone0128@gmail.com/我的雲端硬碟/codex_installation/lazy-pack/)）。**
